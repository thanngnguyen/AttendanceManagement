    ...
    public class AttendanceController : Controller
    {
        ...
        public async Task<IActionResult> CreateSlot(CreateAttendanceSlotViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", $"T?a ?? không h?p l?! Latitude ph?i trong kho?ng [-90, 90], Longitude trong kho?ng [-180, 180]. B?n ?ã nh?p: Lat={model.SlotLatitude}, Lng={model.SlotLongitude}");
                    ViewBag.ClassName = classEntity.ClassName;
                    return View(model);
                }
                ...
                TempData["SuccessMessage"] = $"T?o phiên ?i?m danh thành công! (Lat={model.SlotLatitude:F6}, Lng={model.SlotLongitude:F6})";
        ...
        public async Task<IActionResult> CheckIn(int id)
        {
            ...
            if (!isEnrolled)
            {
                TempData["ErrorMessage"] = "B?n ch?a tham gia l?p h?c này!";
            ...
            if (hasCheckedIn)
            {
                TempData["ErrorMessage"] = "B?n ?ã ?i?m danh r?i!";
                return RedirectToAction(nameof(SlotDetail), new { id });
            }
            ...
            {
                TempData["ErrorMessage"] = "Phiên ?i?m danh ch?a b?t ??u!";
                return RedirectToAction(nameof(SlotDetail), new { id });
            }

            if (DateTime.Now > slot.EndTime)
            {
                TempData["ErrorMessage"] = "Phiên ?i?m danh ?ã k?t thúc!";
                return RedirectToAction(nameof(SlotDetail), new { id });
            }
            ...
            // N?u test mode, t?ng kho?ng cách cho phép
            if (testMode)
            {
        ...
        public async Task<IActionResult> CheckIn(CheckInViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (hasCheckedIn)
                {
                    return Json(new { success = false, message = "B?n ?ã ?i?m danh r?i!" });
                }
                ...
                // Ki?m tra slot có t?a ?? h?p l? không
                ...
                {
                    // Ki?m tra user có t?a ?? h?p l? không
                    ...
                    if (hasValidUserLocation)
                    {
                        ...
                        {
                            isFlagged = true;
                            flagReason = $"Ngoài ph?m vi cho phép: {distance:F2}m (cho phép: {effectiveAllowedDistance}m)";
                    ...
                    else
                    {
                        ...
                        isFlagged = true;
                        flagReason = "Không l?y ???c v? trí h?p l? t? thi?t b?";
                        Console.WriteLine($"? WARNING: User location invalid - Lat={model.Latitude}, Lng={model.Longitude}");
                    }
                }
                else
                {
                    // Slot không có t?a ?? - cho phép ?i?m danh không c?n check v? trí
                    Console.WriteLine("? WARNING: Slot không có t?a ?? GPS - b? qua ki?m tra kho?ng cách");
                    distance = 0; // Set distance = 0 n?u không check
                }
                ...
                if (duplicateDevice)
                {
                    ...
                    if (string.IsNullOrEmpty(flagReason))
                        flagReason = "Thi?t b? ho?c IP trùng v?i sinh viên khác";
                    else
                        flagReason += "; Thi?t b? ho?c IP trùng v?i sinh viên khác";
                ...
                if (isFlagged)
                {
                    if (hasValidSlotLocation && distance > slot.AllowedDistanceMeters)
                    {
                        _context.AttendanceFlags.Add(new AttendanceFlag
                        {
                            ...
                            Reason = $"Kho?ng cách: {distance:F2}m (cho phép: {slot.AllowedDistanceMeters}m)",
                    ...
                    if (duplicateDevice)
                    {
                        _context.AttendanceFlags.Add(new AttendanceFlag
                        {
                            ...
                            Reason = "Thi?t b? ho?c IP trùng v?i sinh viên khác",
                            FlaggedAt = DateTime.UtcNow
                        });
                    }
                ...
                var message = isFlagged 
                    ? $"?i?m danh thành công! L?u ý: {flagReason}" 
                    : "?i?m danh thành công!";
        ...
        public async Task<IActionResult> RequestLeave(int id)
        {
            ...
            if (hasRequested)
            {
                TempData["ErrorMessage"] = "B?n ?ã g?i ??n xin ngh? r?i!";
                return RedirectToAction(nameof(SlotDetail), new { id });
            }
        ...
        public async Task<IActionResult> RequestLeave(RequestLeaveViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                TempData["SuccessMessage"] = "G?i ??n xin ngh? thành công!";
                return RedirectToAction(nameof(SlotDetail), new { id = model.SlotId });
            }
        ...
        public async Task<IActionResult> ReviewLeaveRequest(int requestId, string status, string? note)
        {
            ...
            TempData["SuccessMessage"] = "?ã x? lý ??n xin ngh?!";
